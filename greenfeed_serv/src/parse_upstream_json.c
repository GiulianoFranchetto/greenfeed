...
int parse_upstream_json(iot_push_data data, upstream_packet *packet)
{
    ...
    if (rx_object != NULL) {
        for (i = 0; i < json_array_get_count(rx_object); i++) {
            ...
            base64_decode_block(OR_NULL_STR(json_object_get_string(rxpk_object, "data")), strlen(OR_NULL_STR(json_object_get_string(rxpk_object, "data"))) + 1, pk->payload, &state_in);
